...
import os
import time
sys.path.append('/home/lorenz2/DataEngineeringRepo/DataEngineeringProject/Part4')
from fetcher import Fetcher

def manualLoader(directory):
    dirs = os.listdir(directory)
    files = []
    for f in dirs:
        if 'ascii' in f:
            to_read = '/home/lorenz2/BreadCrumbs/May/'+str(f)
...
if __name__ == '__main__':
    ...
    files = None
    ######## if manually loading 
    #files = ['/home/lorenz2/2022-05-19-ascii']
    #files = manualLoader('../../../BreadCrumbs/May')
    #########
    ...
    if files:
        for f in files:
            try:
                ...
                print('Error loading..', current)
            producer.flush()
            time.sleep(120)
    else:
        produceData(data, 'breadcrumbs', producer, acked)
        data = convertToPacketList(stop_event_data) # re-assign data var to avoid memory overfilling
        produceData(data, 'stopevents', producer, acked)
